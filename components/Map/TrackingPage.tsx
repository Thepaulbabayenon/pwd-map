...
// Dynamic import for the Map component that uses Leaflet
const TrackingMap = dynamic(
  () => import('@/components/Map/TrackingMap'),
  { ssr: false }
);

// Modified person type to work without IDs
export const PersonMapData = {
  firstName: '',
  lastName: '',
  latitude: 0,
  longitude: 0,
...
// Modified facility type to work without IDs
...
// Modified props to accept direct person and facility objects
interface PersonMapDataType {
  firstName: string;
  lastName: string;
  latitude: number;
  longitude: number;
...
interface TrackingPageProps {
  person: PersonMapDataType;
  facility: FacilityDataType;
}
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (distanceToFacility <= 50 && !hasArrived) {
            ...
            // Show arrival toast
            toast.success(`You have arrived at ${facility.name}!`, {
              duration: 5000,
              icon: '🎉',
            });
      ...
      <header className="bg-white shadow-md p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <h1 className="text-xl font-semibold text-center flex-1">
            {navigationStarted 
              ? `Navigating to ${facility.name}` 
              : `Tracking: ${person.firstName} ${person.lastName}`}
          </h1>
            ...
            {!proximityChecked ? (
              ...
              <div className="bg-green-50 border border-green-200 rounded-md p-4 text-center">
                ...
                <p className="text-gray-600">
                  You have successfully arrived at {facility.name}.
                </p>
              </div>
            ) : (
              <div className="bg-blue-50 border border-blue-200 rounded-md p-4 text-center">
                <p className="text-blue-700">
                  Following directions to {facility.name}...
                </p>
                <p className="text-gray-600 mt-2">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-medium capitalize">{facility.type}</p>
            </div>
            <div className="md:col-span-2">
              <p className="text-sm text-gray-500">Coordinates</p>
              <p className="font-medium">{facility.lat}, {facility.lng}</p>
            </div>
          </div>
        </div>
      </div>
