...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed bottom-0 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-4 sm:right-4 sm:top-auto sm:flex-col sm:w-[390px]",
      className
    )}
    {...props}
  />
...
const useToast = () => {
  ...
  const update = React.useCallback(
    (toastId: string, updatedProps: Partial<ToastProps>) => {
      setState((prevState) => 
        prevState.map((t) => 
          t.id === toastId ? { ...t, ...updatedProps } : t
        )
      )
    },
    []
